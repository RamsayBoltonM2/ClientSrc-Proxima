...
#ifdef WJ_SHOW_MOB_INFO
		if (pTextTail->pLevelTextInstance && (pTextTail->bIsPC == FALSE && CPythonSystem::Instance().IsShowMobLevel()))
#else
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, BYTE dwGuildAuth, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
#else
...
#ifdef WJ_GUILD_LEADER_SYSTEM
		if (dwGuildAuth == 3)
			strGuildName.insert(0, "|cFFFF3939");
		else if (dwGuildAuth == 2)
			strGuildName.insert(0, "|cFFFF7E39");
#endif
