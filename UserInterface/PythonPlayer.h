...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	...
	public:
		...
		bool	IsSecurityActivate();
		void	PrepareSecurity(bool bActivate);
#endif
		...
		// Acce
		void	SetAcceItemData(DWORD dwSlotIndex, const TItemData & rItemInstance);
		void	DelAcceItemData(DWORD dwSlotIndex);
		int		GetCurrentAcceSize();
		BOOL	GetAcceSlotItemID(DWORD dwSlotIndex, DWORD* pdwItemID);
		BOOL	GetAcceItemDataPtr(DWORD dwSlotIndex, TItemData ** ppInstance);
	...
	protected:
		...
		DWORD					m_dwCurrentOfflineShopSoulStone;
		DWORD					m_dwCurrentOfflineShopDragonScale;
		DWORD					m_dwCurrentOfflineShopDragonClaw;
#ifdef WJ_CHEQUE_SYSTEM
...
#ifdef WJ_ACCE_SYSTEM
	private:
		typedef std::vector<packet_item> TItemAcceInstanceVector;
