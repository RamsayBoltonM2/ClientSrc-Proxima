...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	if (kChat.dwVID != 0)
	{
		...
		case CHAT_TYPE_DICE_INFO_NORM:
		case CHAT_TYPE_DICE_INFO_COLOR:
#endif
...
bool CPythonNetworkStream::RecvWhisperPacket()
{
	...
	{		
		_snprintf(line, sizeof(line), "> [%d] %s : %s", whisperPacket.bLevel, whisperPacket.szNameFrom, buf);
...
bool CPythonNetworkStream::RecvPointChange()
{
	...
	if (PointChange.dwVID == pInstance->GetVirtualID())
	{
		...
		{
			if (PointChange.amount > 0 && CPythonSystem::Instance().IsShowMoneyText()) // gold
			{
		...
		{
			if (PointChange.amount > 0 && CPythonSystem::Instance().IsExpInfoText())
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnReceiveExp", Py_BuildValue("(i)", PointChange.amount));
			}
		}
	}
...
bool CPythonNetworkStream::SendChannelChangePacket(short sChannel)
{
	if (!__CanActMainInstance())
		return true;
	
	TPacketCGChannelChange p;
	
	p.bHeader	= HEADER_CG_CHANNEL_CHANGE;
	p.sChannel	= sChannel;
	
	if (!Send(sizeof(p), &p))
	{
		Tracef("send_channel_change_packet Error\n");
		return false;
	}
...
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock, BYTE bNpcType)
#else
