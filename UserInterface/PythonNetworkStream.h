...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	public:
		...
		enum
		{
			...
			ACCOUNT_CHARACTER_SLOT_NATIONAL_POINT,
#endif
	...
	public:
		...
		bool SendAddOfflineShopItem(TItemPos bDisplayPos, BYTE bPos, int lPrice, int iPrice);
		...
		bool SendOfflineShopWithdrawMoney(DWORD llMoney);
		...
		// Channel Select
		bool SendChannelChangePacket(short sChannel);
#endif
		...
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock, BYTE bNpcType = 0);
#else
...
#ifdef WJ_OFFLINESHOP_SYSTEM
		// Offline Shop
		bool SendBuildOfflineShopPacket(const char * c_szName, const std::vector<TOfflineShopItemTable> & c_rSellingItemStock, BYTE bNpcType = 0);
#endif
		...
		bool SendPrivateShopSearchInfoSub(int32_t Race, int32_t ItemCat, int32_t SubCat, int32_t MinLevel, int32_t MaxLevel, int32_t MinRefine, int32_t MaxRefine, uint64_t MinGold, uint64_t MaxGold, uint64_t MinCheque, uint64_t MaxCheque, char* ItemName);
	#else
