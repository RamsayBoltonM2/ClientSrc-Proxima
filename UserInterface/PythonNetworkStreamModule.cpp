...
PyObject * netSendAddOfflineShopItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	CPythonNetworkStream & rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendAddOfflineShopItem(TItemPos(bWindowType, bDisplayPos), bPos, iPrice, iPrice2);
	return Py_BuildNone();
}
...
PyObject* netSendChannelChangePacket(PyObject* poSelf, PyObject* poArgs)
{
	int sChannel;
	if (!PyTuple_GetInteger(poArgs, 0, &sChannel))
		return Py_BuildException();
	
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendChannelChangePacket(sChannel);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendPrivateShopSearchInfoSub(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendPrivateShopSearchInfoSub(iJob, iType, iSubType, iMinLevel, iMaxLevel, iMinRefine, iMaxRefine, iMinPrice, iMaxPrice, iMinCheque, iMaxCheque, pcItemName);
#else
	char * pcItemName;
...
void initnet()
{
...
#ifdef WJ_CHANNEL_CHANGE_SYSTEM
		{ "SendChannelChangePacket",			netSendChannelChangePacket,				METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "ACCOUNT_CHARACTER_SLOT_NATIONAL_POINT", CPythonNetworkStream::ACCOUNT_CHARACTER_SLOT_NATIONAL_POINT);
#endif
