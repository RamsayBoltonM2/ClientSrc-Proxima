...
// Eklenenler = 1-) Sash, 2-) Mount
#ifdef ENABLE_NEW_EQUIPMENT_SYSTEM
	const DWORD c_New_Equipment_Start = c_Equipment_Start + 21 + 1 + 1 + 1;
...
#endif
	const DWORD c_Costume_Slot_Count	= 2 + 1 + 1 + 1;
...
const DWORD c_DragonSoul_Equip_Reserved_Count = c_DragonSoul_Equip_Slot_Max * 3;		
	...
	const DWORD c_Inventory_Count	= c_Upgrade_Items_Inventory_Slot_End;
#else
#ifdef ENABLE_NEW_EQUIPMENT_SYSTEM
...
typedef struct SItemPos
{
  ...
  //  }
	...
	{
		bool bResult = 242 <= cell && 258 > cell;
		return bResult;
	}
#endif
