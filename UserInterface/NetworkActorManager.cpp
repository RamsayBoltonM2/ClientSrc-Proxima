...
void SNetworkActorData::__copy__(const SNetworkActorData& src)
{
	...
	m_dwGuildAuth = src.m_dwGuildAuth;
#endif
#ifdef WJ_EVOLUTION_SYSTEM
	m_lWeaponRarity = src.m_lWeaponRarity;
#endif
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor(SNetworkActorData& rkNetActorData)
{
...
#ifdef WJ_GUILD_LEADER_SYSTEM
	kCreateData.m_dwGuildAuth=rkNetActorData.m_dwGuildAuth;
#endif
#ifdef WJ_EVOLUTION_SYSTEM
	kCreateData.m_lWeaponRarity=rkNetActorData.m_lWeaponRarity;
#endif
	...
	if (pNewInstance->GetRace() >= 30000 && pNewInstance->GetRace() <= 30004)
	{
		CPythonCharacterManager & rkChrMgr = CPythonCharacterManager::Instance();
		CInstanceBase* pkInstMain = rkChrMgr.GetMainInstancePtr();
		CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();

		if (CPythonSystem::Instance().IsShowShops() == false)
		{
			pNewInstance->Hide();
			rkChrMgr.SCRIPT_SetAffect(pNewInstance->GetVirtualID(), 1, true);
			//rkNetStream.DisappearShopSign(pNewInstance->GetVirtualID());
		}
	}
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (pkInstFind)
	{
		...
		pkInstFind->ChangeWeapon(c_rkNetUpdateActorData.m_dwWeapon, c_rkNetUpdateActorData.m_lWeaponRarity);
#elif !defined(WJ_EVOLUTION_SYSTEM) and defined(WJ_QUIVER_SYSTEM)
...
#endif	
#ifdef WJ_GUILD_LEADER_SYSTEM
		pkInstFind->ChangeGuild(c_rkNetUpdateActorData.m_dwGuildID, c_rkNetUpdateActorData.m_dwGuildAuth);
#else
		...
		pkInstFind->SetLevel(c_rkNetUpdateActorData.m_dwLevel);	// level update fix
		pkInstFind->SetPKMode(c_rkNetUpdateActorData.m_byPKMode);
		pkInstFind->SetStateFlags(c_rkNetUpdateActorData.m_dwStateFlags);
#ifdef WJ_EVOLUTION_SYSTEM
		pkInstFind->SetWeaponRarity(c_rkNetUpdateActorData.m_lWeaponRarity);
#endif
