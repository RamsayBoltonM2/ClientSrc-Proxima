...
PyObject * textTailRegisterCharacterTextTail(PyObject * poSelf, PyObject * poArgs)
{
	...
	int iGuildAuth;
	if (!PyTuple_GetInteger(poArgs, 1, &iGuildAuth))
		return Py_BuildException();
#endif
	int iVirtualID;
...
#ifdef WJ_GUILD_LEADER_SYSTEM
	CPythonTextTail::Instance().RegisterCharacterTextTail(iGuildID, iGuildAuth, iVirtualID, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
#else
