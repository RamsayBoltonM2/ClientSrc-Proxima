...
void CSlotWindow::ActivateAcceSlot(DWORD dwIndex)
{
	...
	#ifdef WJ_HIGHLIGHT_SYSTEM
	if (!m_apSlotAcceActiveEffect[0] || !m_apSlotAcceActiveEffect[1] || !m_apSlotAcceActiveEffect[2])
	#else
...
void CSlotWindow::ActivateChangeLookSlot(DWORD dwIndex)
{
	...
	#ifdef WJ_HIGHLIGHT_SYSTEM
	if (!m_apSlotChangeLookActiveEffect[0] || !m_apSlotChangeLookActiveEffect[1] || !m_apSlotChangeLookActiveEffect[2])
	#else
	if (!m_pSlotChangeLookActiveEffect)
	#endif
	{
		__CreateChangeLookSlotEnableEffect();
	}
}
...
void CSlotWindow::ClearSlot(TSlot * pSlot)
{
	...
	pSlot->sDiffuseColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 0.5f);
}
...
void CSlotWindow::OnUpdate()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_apSlotAcceActiveEffect[i])
			m_apSlotAcceActiveEffect[i]->Update();
	}
	#else
	if (m_pSlotAcceActiveEffect)
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_apSlotChangeLookActiveEffect[i])
			m_apSlotChangeLookActiveEffect[i]->Update();
	}
	#else
	if (m_pSlotChangeLookActiveEffect)
		m_pSlotChangeLookActiveEffect->Update();
	#endif
	if (m_pSlotCoverImage)
...
void CSlotWindow::OnRender()
{
	...
	for (itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (rSlot.bAcceActive)
		{
			...
			if (highlight == 1)
			{
				m_apSlotAcceActiveEffect[0]->Show();
				m_apSlotAcceActiveEffect[0]->SetPosition(ix, iy);
				m_apSlotAcceActiveEffect[0]->Render();
			}
			
			if (highlight == 2)
			{
				m_apSlotAcceActiveEffect[1]->Show();
				m_apSlotAcceActiveEffect[1]->SetPosition(ix, iy);
				m_apSlotAcceActiveEffect[1]->Render();
			}
			
			if (highlight == 3)
			{
				m_apSlotAcceActiveEffect[2]->Show();
				m_apSlotAcceActiveEffect[2]->SetPosition(ix, iy);
				m_apSlotAcceActiveEffect[2]->Render();
			}
			#else
			if (m_pSlotActiveEffect)
			{
		...
		if (rSlot.bChangeLookActive)
		{
			...
			if (highlight == 1)
			{
				m_apSlotChangeLookActiveEffect[0]->Show();
				m_apSlotChangeLookActiveEffect[0]->SetPosition(ix, iy);
				m_apSlotChangeLookActiveEffect[0]->Render();
			}
			
			if (highlight == 2)
			{
				m_apSlotChangeLookActiveEffect[1]->Show();
				m_apSlotChangeLookActiveEffect[1]->SetPosition(ix, iy);
				m_apSlotChangeLookActiveEffect[1]->Render();
			}
			
			if (highlight == 3)
			{
				m_apSlotChangeLookActiveEffect[2]->Show();
				m_apSlotChangeLookActiveEffect[2]->SetPosition(ix, iy);
				m_apSlotChangeLookActiveEffect[2]->Render();
			}
			#else
			if (m_pSlotActiveEffect)
			{
...
void CSlotWindow::__CreateAcceSlotEnableEffect()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		{
			char buf[64+1];
			sprintf_s(buf, "d:/ymir work/ui/public/acceactiveeffect/slot%d/%02d.tga", (i+1), j);
			pEff->AppendImage(buf);
		}
		
		pEff->SetRenderingMode(CGraphicExpandedImageInstance::RENDERING_MODE_SCREEN);
		m_apSlotAcceActiveEffect[i] = pEff;
	}
	#else
	m_pSlotAcceActiveEffect = new CAniImageBox(NULL);
	m_pSlotAcceActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotAcceActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotAcceActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotAcceActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotAcceActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
...
void CSlotWindow::__DestroySlotAcceEnableEffect()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_apSlotAcceActiveEffect[i])
		{
			delete m_apSlotAcceActiveEffect[i];
			m_apSlotAcceActiveEffect[i] = NULL;
		}
	}
	#else
	if (m_pSlotAcceActiveEffect)
...
#ifdef WJ_CHANGELOOK_SYSTEM
void CSlotWindow::__CreateChangeLookSlotEnableEffect()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		{
			char buf[64+1];
			sprintf_s(buf, "d:/ymir work/ui/public/changelookactiveeffect/slot%d/%02d.tga", (i+1), j);
			pEff->AppendImage(buf);
		}
		
		pEff->SetRenderingMode(CGraphicExpandedImageInstance::RENDERING_MODE_SCREEN);
		m_apSlotChangeLookActiveEffect[i] = pEff;
	}
	#else
	m_pSlotChangeLookActiveEffect = new CAniImageBox(NULL);
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	...
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotChangeLookActiveEffect->AppendImage("d:/ymir work/ui/public/slotactiveeffect/00.sub");
	m_pSlotChangeLookActiveEffect->SetRenderingMode(CGraphicExpandedImageInstance::RENDERING_MODE_SCREEN);
	m_pSlotChangeLookActiveEffect->Show();
	#endif
}
...
void CSlotWindow::__DestroySlotChangeLookEnableEffect()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		if (m_apSlotChangeLookActiveEffect[i])
		{
			delete m_apSlotChangeLookActiveEffect[i];
			m_apSlotChangeLookActiveEffect[i] = NULL;
		}
	}
	#else
	if (m_pSlotChangeLookActiveEffect)
	{
		delete m_pSlotChangeLookActiveEffect;
		m_pSlotChangeLookActiveEffect = NULL;
	}
	#endif
}
#endif
